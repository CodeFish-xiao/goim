...
import (
	...
	"github.com/Terry-Mao/goim/services/logic/internal/dao"
	model2 "github.com/Terry-Mao/goim/services/logic/internal/model"
	"strconv"
	"time"
	...
	if zoneIns, ok := res.Fetch(); ok {
		...
		for _, zins := range zoneIns.Instances {
			for _, ins := range zins {
				...
				offline, err := strconv.ParseBool(ins.Metadata[model2.MetaOffline])
				if err != nil || offline {
					log.Warningf("strconv.ParseBool(offline:%t) error(%v)", offline, err)
					continue
				}
				conns, err := strconv.ParseInt(ins.Metadata[model2.MetaConnCount], 10, 32)
				if err != nil {
					log.Errorf("strconv.ParseInt(conns:%d) error(%v)", conns, err)
					continue
				}
				ips, err := strconv.ParseInt(ins.Metadata[model2.MetaIPCount], 10, 32)
				if err != nil {
